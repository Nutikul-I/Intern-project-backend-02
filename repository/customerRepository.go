...
func GetCustomerRepository(mid string, page int, row int) ([]model.CustomerPayload, error) {
	conn := ConnectDB()
	...
	var customers []model.CustomerPayload
	err = scan.Rows(&customers, rows)
	if err != nil {
...
func GetTotalCustomerRepository(row int) (int, error) {
	conn := ConnectDB()
	ctx := context.Background()

	var totalCount int
	err := conn.QueryRowContext(ctx, model.SQL_COUNT_CUSTOMER).Scan(&totalCount)
	if err != nil {
		log.Errorf("Error counting customers: %v", err)
		return 0, err
	}
